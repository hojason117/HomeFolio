...
func NewHandler(session *sql.DB, dbURL string) (h *Handler) {
	h = &Handler{websocket.Upgrader{CheckOrigin: func(r *http.Request) bool { return true }}, clientManager{make(map[string]map[uuid.UUID]*client),
		make(chan model.Notification), make(chan *client), make(chan *client), session, dbURL}}
...
func (h *Handler) GetConnection(c echo.Context) (err error) {
	...
	id, err := uuid.NewV1()
	if err != nil {
		return &echo.HTTPError{Code: http.StatusBadRequest, Message: "Failed to generate uuid."}
	}
